...
class CCD:
    ...
    def get_token(self):
        ...
        token, ticket = self.extract_token(redirect_url)
        if token:
            self.zhc_token = token
            self.auth_login(token)
            if ticket:
                self.dmsp_ticket = ticket
    ...
    def extract_token(self, redirect_url):
        ...
        start_ticket_index = redirect_url.find("__dmsp_ticket=") + len("__dmsp_ticket=")

        token = None
        ticket = None
        if start_token_index != -1 and end_token_index != -1:
            token = redirect_url[start_token_index:end_token_index]
        if start_ticket_index != -1:
            ticket = redirect_url[start_ticket_index:]
        return token, ticket
    ...
    def get_csrftoken(self):
        url1 = f'https://fission-events.ccbft.com/a/224/kmenz5Zd?CCB_Chnl=6000110&__dmsp_token={self.zhc_token}&__dmsp_ticket={self.dmsp_ticket}'
    ...
    # 消保专区
    def fire(self):
        url1 = 'https://fission-events.ccbft.com/activity/dmspxbmountain/getUserInfo/224/8ZWX263w'  # 游戏次数
        return_data1 = self.send_request(url1, headers = self.bus_headers)
        self.sleep()
        if return_data1['status'] != 'success':
            print(return_data1['message'])
        ...
        for _ in range(int(remain_num)):
            url2 = 'https://fission-events.ccbft.com/activity/dmspxbmountain/startChallenge/224/8ZWX263w'  # 游戏id
            return_data2 = self.send_request(url2, headers = self.bus_headers, method = 'POST')
            if return_data2['status'] != 'success':
                print(return_data2['message'])
                return
            game_id = return_data2['data']
            time.sleep(2)
            url = 'https://fission-events.ccbft.com/Common/activity/getActivityInfo/224/8ZWX263w'
            ...
            print('获取成功，开始登山游戏')
            url3 = 'https://fission-events.ccbft.com/activity/dmspxbmountain/doChallenge/224/8ZWX263w'  # 开始游戏
            payload1 = {"stage": 20, "score": random.randint(200, 250), "l_id": game_id}
            return_data3 = self.send_request(url3, headers = self.bus_headers, data = payload1, method = 'POST')
            ...
            self.sleep()
            if return_data4['status'] != 'success':
